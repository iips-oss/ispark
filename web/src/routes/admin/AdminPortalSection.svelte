  ...
  <aside class="lg:col-span-5 flex flex-col gap-6 animate-fade-in">
    <div class="flex flex-col gap-4">
      ...
      <p class="text-slate-500 text-sm leading-relaxed">
        Secure access for authorized iSPARC administrators responsible for
        monitoring student participation records, activity verification,
        grading workflows, and institutional reporting.
      </p>
    </div>
    ...
    <div class="flex flex-col gap-4 bg-white p-6 rounded-xl border border-border-base shadow-xs">
      <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">System Privileges</h3>
      
      <div class="flex flex-col gap-5">
        {#each featureItems as item}
          <div class="flex items-start gap-4">
            <div class="flex w-9 h-9 items-center justify-center shrink-0 bg-slate-50 rounded-lg border border-border-base text-inst-navy">
      ...
      <div class="p-8 sm:p-12 text-center flex flex-col items-center gap-6" in:fade={{ duration: 400 }}>
        ...
        <div>
          ...
          <p class="text-slate-500 text-sm mt-2 max-w-md mx-auto">
            Welcome, Administrator <span class="font-semibold text-slate-800">{adminId}</span>. You have logged into the iSPARC Administrative Panel. Redirecting to administrative console...
          </p>
        </div>
    ...
    {:else}
      ...
      <div class="p-6 sm:p-8 border-b border-border-base bg-slate-50/50">
        ...
        <p class="text-slate-500 text-xs mt-1">
          Login to access the iSPARC Administration Portal
        </p>
      </div>
      ...
      <form onsubmit={handleSubmit} class="p-6 sm:p-8 flex flex-col gap-6">
        ...
        <div class="flex flex-col gap-1.5">
          <label for="{formId}-admin-id" class="text-[11px] font-bold text-slate-700 tracking-wider">
            ADMIN ID
          </label>
          <input
            id="{formId}-admin-id"
